...
-- 5. Identify tables with similar names (potential duplicates)
WITH table_names AS (
    SELECT tablename
    FROM pg_tables 
    WHERE schemaname = 'public'
...
SELECT 
    ...
    similarity(t1.tablename, t2.tablename) as similarity_score
FROM table_names t1
CROSS JOIN table_names t2
WHERE t1.tablename != t2.tablename
    AND similarity(t1.tablename, t2.tablename) > 0.6
