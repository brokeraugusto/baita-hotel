-- Analyze maintenance-related tables for duplicates and obsolete versions (FIXED)
...
-- 6. Check for maintenance template tables
SELECT 
    table_name,
    column_name,
    data_type
FROM information_schema.columns
WHERE table_schema = 'public'
    AND table_name LIKE '%maintenance%template%'
ORDER BY table_name, ordinal_position;
