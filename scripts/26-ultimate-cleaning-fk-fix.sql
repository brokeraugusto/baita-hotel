...
-- E DIAGNÓSTICO FINAL ANTES DE RE-ADICIONAR FKs
DO $$
DECLARE
...
BEGIN
    ...
    SELECT COUNT(*) INTO personnel_count FROM cleaning_personnel;

    -- Limpar cleaning_personnel.hotel_id
    IF column_exists('cleaning_personnel', 'hotel_id') THEN
        IF hotel_count = 0 THEN
            -- Se não há hotéis, todos os hotel_id em cleaning_personnel devem ser NULL
            UPDATE cleaning_personnel SET hotel_id = NULL WHERE hotel_id IS NOT NULL;
        ELSE
    ...
    -- Limpar cleaning_tasks.hotel_id
    IF column_exists('cleaning_tasks', 'hotel_id') THEN
        IF hotel_count = 0 THEN
            UPDATE cleaning_tasks SET hotel_id = NULL WHERE hotel_id IS NOT NULL;
        ELSE
    ...
    -- Limpar cleaning_tasks.room_id
    IF column_exists('cleaning_tasks', 'room_id') THEN
        IF rooms_count = 0 THEN
            UPDATE cleaning_tasks SET room_id = NULL WHERE room_id IS NOT NULL;
        ELSE
            UPDATE cleaning_tasks
            SET room_id = NULL
    ...
    -- Limpar cleaning_tasks.assigned_personnel_id
    IF column_exists('cleaning_tasks', 'assigned_personnel_id') THEN
        IF personnel_count = 0 THEN
            UPDATE cleaning_tasks SET assigned_personnel_id = NULL WHERE assigned_personnel_id IS NOT NULL;
        ELSE
            UPDATE cleaning_tasks
    ...
    -- Preencher NULLs para colunas NOT NULL que dependem de fallback
    ...
    -- DIAGNÓSTICO FINAL: Verificar se ainda há IDs órfãos antes de tentar adicionar FKs
    -- Estes SELECTs serão executados e seus resultados (se houver) serão visíveis no log de execução.
    -- Se qualquer um retornar linhas, a FK falhará.
    SELECT 'cleaning_personnel.hotel_id órfãos restantes:' AS diagnostic_message, id, hotel_id FROM cleaning_personnel
    WHERE hotel_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM hotels WHERE hotels.id = cleaning_personnel.hotel_id);

    SELECT 'cleaning_tasks.hotel_id órfãos restantes:' AS diagnostic_message, id, hotel_id FROM cleaning_tasks
    WHERE hotel_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM hotels WHERE hotels.id = cleaning_tasks.hotel_id);

    SELECT 'cleaning_tasks.room_id órfãos restantes:' AS diagnostic_message, id, room_id FROM cleaning_tasks
    WHERE room_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM rooms WHERE rooms.id = cleaning_tasks.room_id);

    SELECT 'cleaning_tasks.assigned_personnel_id órfãos restantes:' AS diagnostic_message, id, assigned_personnel_id FROM cleaning_tasks
    WHERE assigned_personnel_id IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM cleaning_personnel WHERE cleaning_personnel.id = cleaning_tasks.assigned_personnel_id);
