...
) AS v(name, slug, description, price_monthly, price_yearly, features, limits, max_hotels, max_rooms, max_users, max_integrations, is_active, is_featured, sort_order)
WHERE NOT EXISTS (
    SELECT 1 FROM public.subscription_plans WHERE slug = v.slug
);
