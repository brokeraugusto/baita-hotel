...
BEGIN
    ...
    LOOP
        INSERT INTO public.profiles (
            ...
            backup_record.id,
            backup_record.email,
            COALESCE(backup_record.full_name, backup_record.name),
            COALESCE(backup_record.user_role, backup_record.role, 'client')::user_role,
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE '   ⚠️ Erro na restauração: %', SQLERRM;
        RAISE NOTICE '   Continuando com criação de usuários padrão...';
        RAISE NOTICE '';
END $$;
