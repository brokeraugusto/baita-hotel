...
export const clientsService = {
  async getAll(): Promise<Client[]> {
    try {
      ...
      // Get profiles with client role and their hotels
  ...
  async create(clientData: {
    ...
    try {
      ...
      const currentUser = await productionAuthService.getCurrentUser()
      if (!currentUser) {
      ...
      if (currentUser.role !== "master_admin") {
        console.error("❌ User is not master admin:", currentUser.role)
        throw new Error("Only master admin can create client users")
      }

      console.log("✅ Master admin verified for client creation:", currentUser.email)
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      // Delete in order: subscriptions -> hotels -> profiles -> auth
      // The RLS policies and foreign keys should handle cascading
